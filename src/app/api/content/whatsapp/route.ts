...
import { generateWhatsApp, type ContentContext } from "@/lib/ai/content";
...
/**
 * POST /api/content/whatsapp — AI-generate a WhatsApp message, optionally
 * personalised from a company's analysis + decision makers.
 */
...
export async function POST(req: Request) {
  return withApi(async () => {
    ...
    const body = await generateWhatsApp(category, ctx);
    ...
    await logActivity({
      type: "WHATSAPP_GENERATED",
      message: `${user.name} generated a ${category.toLowerCase().replace(/_/g, " ")} WhatsApp message${
